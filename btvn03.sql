...
-- Tham số  đầu vào(IN) : p_total_price DECIMAL(15,2), p_type VARCHAR(100)
-- Tham số đầu ra(OUT): p_money_spent DECIMAL(15,2), p_thongbao VARCHAR (100)

-- Các bước nên thực hiện
-- b1 kiểm tra tính hợp lệ của total_price, money_spent
-- b2 tính toán phân loại BHYT, VIP, THUONG
-- b3 Gán thông báo vào biến đầu ra và call

DELIMITER //

CREATE PROCEDURE sp_hospital_price(
    IN p_total_price DECIMAL(18,2),
    IN p_type VARCHAR(20),
    OUT p_money_spent DECIMAL(18,2),
    OUT p_thongbao VARCHAR(255)
)
	BEGIN
		IF p_total_price < 0 THEN
			SET p_money_spent = 0;
			SET p_thongbao = 'Chi phí không hợp lệ';
		ELSE
			IF p_type = 'BHYT' THEN 
				SET p_money_spent = p_total_price * 0.2;
			ELSEIF p_type = 'VIP' THEN 
				SET p_money_spent = p_total_price * 0.9;
			ELSEIF p_type = 'THUONG' THEN 
				SET p_money_spent = p_total_price;
			ELSE 
				SET p_money_spent = p_total_price;
			END IF;
            
			SET p_thongbao = 'Đã tính toán xong';
		END IF;
	END 
//DELIMITER ;
...
-- TH1: BHYT
CALL sp_hospital_price(1000000, 'BHYT', @tien_thu, @msg);
SELECT 'BHYT' AS Loai, @tien_thu, @msg;

-- TH2: VIP
CALL sp_hospital_price(1000000, 'VIP', @tien_thu, @msg);
SELECT 'VIP' AS Loai, @tien_thu, @msg;

-- TH3: THUONG
CALL sp_hospital_price(1000000, 'THUONG', @tien_thu, @msg);
SELECT 'THUONG' AS Loai, @tien_thu, @msg;

-- TH4: LOI
CALL sp_hospital_price(-500000, 'BHYT', @tien_thu, @msg);
SELECT 'LOI' AS Loai, @tien_thu, @msg;